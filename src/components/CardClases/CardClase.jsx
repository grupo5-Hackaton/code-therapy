...
const CardClase = () => {
  ...
  const getAllDetalles = async () => {
    //Funcion para coger todos los datos de la api
      ...
      <section id="experience">
        ...
        {detalle.map((proyecto) => (
          <div className="separacion" key={proyecto.id}>
            {" "}
            {/* se pone como key la id del objeto */}
            <Link to={`/DetalleClase/${proyecto.id}`}>
              {" "}
              {/* Tambien se para el id para la pagina personalizada */}
              <div className="card">
                <Card>
            ...
            {/* Se hace un link(a href) para solo tener que hacer clic en la card*/}
          </div>
        ))}
      </section>
    </div>
